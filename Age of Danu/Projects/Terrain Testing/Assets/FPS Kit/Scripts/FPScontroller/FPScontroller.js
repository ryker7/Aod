...
function Prone(){
	var up = transform.TransformDirection(Vector3.up);
   	...
   	if(!Physics.Raycast(transform.position, up, hit, movement.crouchHeight)){ 
		if(inputProne && !Running && !onLadder && !canDo){
			CanDo();
			prone = !prone;
			crouch = false;
		}
...
function Crouch(){
	var up = transform.TransformDirection(Vector3.up);
   	...
   	if(!Physics.Raycast(transform.position, up, hit, movement.crouchHeight*2)||prone){ 
		if(inputCrouch && ! Running && !onLadder && !canDo){
			CanDo();
			crouch = !crouch;
			prone = false;
		}
...
function CanDo(){
	canDo = true;
	yield WaitForSeconds(movement.crouchSmooth*2);
	canDo = false;
}
...
@script AddComponentMenu ("FPS system/FPS Controller")
